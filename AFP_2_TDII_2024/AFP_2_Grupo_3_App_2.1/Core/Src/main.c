...
int main(void)
{
  ...
  uint16_t vector_Pin_LEDs[CANTIDAD_LEDS] = {LD1_Pin, LD2_Pin, LD3_Pin};	//Vector con los pines asignados a los distintos leds
  ...
  while (1)
  {
    ...
    for (int i = 0; i < CANTIDAD_LEDS; i++) {
        toggleLed_GPIO(vector_Pin_LEDs[i]); // Cambiar el estado del LED i
        delay_ms(retardo_ms);         		// Retardo usando el driver
        toggleLed_GPIO(vector_Pin_LEDs[i]); // Volver a cambiar el estado del LED i
        delay_ms(retardo_ms);         		// Retardo usando el driver
    }
